...
internal FastAPI backend under `/api/*` and streamed from FastAPI over WebSockets or
...
scheduler, data, schemas, and evaluation internals; `api` owns the internal
FastAPI relay; `web` owns the Next.js SPA source; and `cli` owns the Typer CLI (see
...
- **`api`** — the internal FastAPI relay under `src/api`. Owns `/api/*` scenario
...
- **`web`** — the Next.js static-export SPA under `src/web` using Shadcn UI
  ...
  internal FastAPI snapshot endpoints and realtime streams under `/api/*`.
...
`api` command entrypoints, and the `web` app source depends only on the internal
`/api/*` HTTP/realtime boundary, not Python internals.
...
SPA/static export. It owns `/*`, while the internal FastAPI relay owns
`/api/*`. The exported assets are intentionally dumb/static: they do not embed a
...
process (started via TraCI), the internal FastAPI relay in `src/api` (serving
...
├── api/
│   ├── __init__.py
│   ├── app.py                       # FastAPI app factory; mounts internal routes under /api/*
│   ├── routes.py                    # internal /api/* endpoints: scenarios, controls, snapshots, reports
...
- FastAPI documentation. [*Bigger Applications - Multiple Files*](https://fastapi.tiangolo.com/tutorial/bigger-applications/). — Grounds using
  routers and a single `/api` prefix at the internal application boundary (see
  [Internal API and dashboard](#internal-api-and-dashboard)).
