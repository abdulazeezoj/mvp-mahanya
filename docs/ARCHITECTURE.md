...
`api/` and `web/` are both implemented. This document specifies the
source layout and component design they were built against, and remains the
...
flowchart LR
    ...
    subgraph MaHanya
        direction TB
        Monitor(["Monitor live junction state"])
        Log(["View decision log (reason codes)"])
        Eval(["Run evaluation vs. fixed-time baseline"])
        Recommend(["Recommend signal phase (model)"])
        Validate(["Validate & apply signal decision (scheduler)"])
        Preempt(["Receive priority pre-emption"])
    end
    ...
    Operator --> Eval
    Emergency --> Preempt
    Recommend -. include .-> Validate
    Preempt -. "extend (short-circuits the model)" .-> Validate
```
...
| Web toolchain | Next.js configured for static SPA export, managed with `pnpm` | Provides a modern TypeScript/React UI while still producing static assets served at `/*`; `web/` is a fully self-contained Next.js project with its own `package.json` and `web/src` source tree. |
...
PyTorch wheel index; `api/pyproject.toml` uses the `uv` equivalent — a
`[[tool.uv.index]]` entry naming the PyTorch CPU wheel index, combined with a
`[tool.uv.sources]` entry pinning `torch` to it.
...
- The `api/`/`web/` split maps to independently deployable units and
  independent toolchains (`uv` vs `pnpm`): each can be built, tested, and
...
Implemented as documented below (kept as "proposed" in the heading only for
link stability with other docs that reference `#proposed-source-layout`).
...
No CI workflow exists yet. `api/` (`uv run pytest`) and `web/` (`pnpm test`,
`pnpm test:e2e`) both have test suites a workflow could run; it just hasn't
been requested or set up.
